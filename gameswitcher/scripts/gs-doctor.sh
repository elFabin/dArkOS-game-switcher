...
# whether nc/python3 are present, the configured Fn/hotkey code and
...
echo "-- Tools --"
# ffmpeg used to be checked here -- gameswitcher.c decodes PNG thumbnails
# itself now (src/png.h), so nothing in the switch path depends on it any
# more. nc stays: gs_ra_cmd still falls back to it if this bash build lacks
# /dev/udp support.
